...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_help_command(self):
        """Tests the help command. Default `do_help()` logic
        uses `self.stdout` (which defaults to `sys.__stdout__`)
        not `sys.stdout` hence the patch to different stdout
        """
        with patch.object(self.cmd, "stdout", new=StringIO()) as f:
...
"""
        self.assertEqual(f.getvalue(), string)

    def test_prompt(self):
        """Tests that prompt is correctly displayed as `(hbnb) `"""
        with patch("builtins.input") as mock_input:
            mock_input.side_effect = ["exit"]
            self.cmd.cmdloop()
            mock_input.assert_called_with("(hbnb) ")
